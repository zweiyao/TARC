...
def main(_):
    ...
    env = config.get_environment(
        ...
        # Reward comes from KeyboardRewardWrapper, which tarc's get_environment
        # always attaches; there is no classifier checkpoint to load.
        classifier=False,
    )
    ...
    if config.setup_mode == 'single-arm-fixed-gripper' or config.setup_mode == 'dual-arm-fixed-gripper':   
        agent: SACAgent = make_sac_pixel_agent(
            ...
            # Without these two the observation still carries tactile and the
            # action chunk, but the encoder never reads them: the actor's first
            # layer comes out 576 wide instead of 896 and the policy trains on
            # the cameras alone, with nothing raised anywhere.
    ...
    if FLAGS.checkpoint_path is not None and os.path.exists(FLAGS.checkpoint_path):
        # The original blocks on input() here, which hangs an unattended run.
        print_green(f"Checkpoint path {FLAGS.checkpoint_path} exists; resuming.")
        ckpt = checkpoints.restore_checkpoint(
            os.path.abspath(FLAGS.checkpoint_path),
            agent.state,
        )
